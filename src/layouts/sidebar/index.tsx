				...
				>
					<div className="flex items-center justify-end w-full">
						<button
							...
							className="p-1 rounded-md hover:bg-sidebarhover hover:text-red-500"
						>
					...
					<div className="flex flex-col gap-2">
						{items.map((item) => (
							<NavLink
								key={item.id}
								to={item.href}
								className={`${item.current ? "bg-sidebarhover border-b-2 border-purple-600" : ""} p-2 rounded-md hover:bg-sidebarhover hover:text-red-500`}
					...
					{items.map((item) => (
						<>
							<NavLink
								...
								to={item.href}
								onClick={() => handleItemClick(item.id)}
								className={`w-full rounded-md hover:bg-sidebarhover py-2 px-2 truncate text-sidebartext ${item.current ? " border-b-2 border-purple-600" : "boerder-b-2 border-transparent"}`}
							>
								{item.name}
							</NavLink>
						</>

...
function SearchIbge() {
	...
	return (
		<>
			...
			<div>
				<FirstTitle>
					Instituto Brasileiro de Geografia e Estatística (Código IBGE)
				</FirstTitle>
			</div>
			<div className="w-full flex items-center justify-center gap-4">
				<div className="grid grid-cols-1 md:grid-cols-[2fr_2fr_1fr] gap-2 items-center">
					<div className="grid-cols-1 md:flex items-center gap-2">
						...
						<Select onValueChange={handleChange}>
							...
							<SelectContent>
								{estados.map((item) => {
									return (
										<SelectItem key={item.id} value={item.uf}>
											{item.name}
										</SelectItem>
									);
								})}
					...
					<Input
						...
						placeholder="Filtre pela cidade (Opcional)"
					/>
					<Button onClick={getCitys}>
						<Search />
						Buscar
					</Button>
				</div>
